...
#show: report.with(
  ...
  abstract: [
    ...
    The intensity factor ($-67 "," 366$ TJ) dominates, driven by building
    envelope improvements (~45%), fuel switching from oil to heat pumps (~35%),
    and behavioural changes (~20%). However, improvement is decelerating: the
    annual intensity gain has moderated from $-3 "," 200$ TJ/yr in the 2000s
    to $-2 "," 500$ TJ/yr in the 2020s. Analysis of GWR building-register
    data reveals a four-speed heat-pump transition across building types (EFH
    28%, MFH 20%) and a 34-percentage-point spread across cantons. We identify
...
accounting for approximately 65% of total residential energy consumption in
...
The IPCC AR5 Chapter 9 concludes that buildings account for roughly
...
improving at ~1.5%/yr, largely offset by floor-area growth.
...
- *Intensity attribution*: The sub-decomposition of the intensity effect
  into envelope, fuel-switching, and other components is indicative, based
  on engineering estimates from @prognos2024 and @kaempf2018, not a formal
...
over half of residential energy --- have roughly halved in absolute
...
The carrier mix has shifted from a fossil-dominated system (>70% oil and
...
Heating degree days (HDD) have declined at approximately $-156$ HDD per
...
indicate continued growth to approximately 10.4 million by 2050. This
...
oil has declined from 59.9% (2000) to 23.9% (2024), while heat pumps
have risen from near zero to 28.9%. Gas has remained stable at around
19--21%; wood and district heating together account for approximately 12%.
...
Single-family homes (EFH) provide approximately 55 m#super[2]/person,
while multi-family buildings (MFH) provide roughly 43 m#super[2]/person
--- a persistent 28% gap (@fig:floor-period). The population-weighted
average of approximately 46.6 m#super[2]/person has remained remarkably
...
intensity effect has moderated from roughly $-3 "," 200$ TJ/yr in the
...
aggregates several distinct mechanisms. @tab:intensity-attr presents
an indicative attribution based on engineering estimates and
literature values.
...
Heat pumps (COP ~3.5) consume roughly 70% less final energy than oil
boilers for the same useful heat. As @fig:heating-mix shows, heat-pump
floor-area share has risen from near zero to 28.9%, driving a substantial
...
#insight[
  Building envelope improvements account for approximately 45% of
  the intensity decline ($approx -30 "," 300$ TJ), but the current
  renovation rate of 1.1%/year is roughly half the 2%/year needed
...
#figure(
  ...
  with 28% HP share; mixed-use and partial-residential buildings lag
  significantly. Source: BFS GWR @bfs2024.],
) <fig:heating-type>
...
#figure(
  ...
  caption: [Heat-pump adoption rates by building type, 2021--2024.
...
@fig:floor-period shows that EFH provide approximately 55
...
requires heating for roughly 12 m#super[2] more floor area than their
...
Critically, non-EFH buildings (MFH, mixed-use, partial) house approximately
73% of the Swiss population but account for only 20--30% of heat-pump
installations. Unlocking this segment is essential for maintaining the pace
...
#figure(
  ...
  HP as primary heating system), 2024. Fribourg leads at 41.8%,
  followed by Luzern (33.2%) and Aargau (28.9%). Basel-Stadt (7.4%)
  and Neuchâtel (12.0%) lag. Source: BFS GWR @bfs2024 @sdmx2024.],
) <fig:canton-map>

Fribourg leads at 41.8%, while Basel-Stadt (7.4%) and Neuchâtel (12.0%)
lag substantially --- a five-fold spread reflecting differences in building
...
contribution averaged roughly $-3 "," 200$ TJ/year during the 2000s
but has moderated to approximately $-2 "," 500$ TJ/year in the 2010s
and early 2020s. Several factors explain this deceleration:
...
- *Slow retrofit rate*: At 1.1%/year, the existing stock turns over
  ...
  oil boilers in EFH with heat pumps --- are increasingly exhausted,
...
- *Floor-area rebound*: Some of the efficiency gains have been offset
  ...
  particularly in the EFH segment.
...
The full quantitative summary is presented in @conclusion.
...
2--2.5%/yr --- roughly double the current pace @tep2023.
...
renewable share when replacing heating systems. The partial adoption creates
...
== Monte Carlo Methodology: Why Uniform Distributions <monte-carlo>

We complement the three named scenarios with 1,000 Monte Carlo iterations
sampling key parameters from defined ranges to produce uncertainty envelopes.
...
#figure(
  table(
    ...
    table.header(
      [*Parameter*], [*BAU*], [*Accelerated*], [*Ambitious*], [*MC sampling range*],
    ),
    table.hline(stroke: 0.8pt),
    [Renovation rate (%/yr)], [1.0], [2.0], [3.0], [0.8--3.2 (uniform)],
    [Population growth], [BFS ref.], [BFS ref.], [BFS ref.], [BFS low--high (uniform)],
    [HDD decline (%/decade)], [2.5], [2.5], [2.5], [$mu = 2.5$, $sigma = 1.0$ (normal)],
    table.hline(stroke: 0.8pt),
  ),
  caption: [Monte Carlo parameter ranges. Population is sampled uniformly
  between BFS high and low projection scenarios; renovation rate is sampled
  uniformly between 0.8--3.2%/yr.],
) <tab:mc-params>

=== Why Uniform Distributions

*Epistemic, not aleatoric uncertainty.* Future renovation rates and
population trajectories are outcomes of policy choices and political
decisions, not random variables with a known distribution. Uniform
distributions express maximum ignorance within calibrated bounds.

*Conservative choice.* Equal weighting across the range produces wider
confidence bands than triangular or normal distributions, appropriately
reflecting genuine uncertainty over a 25-year horizon where structural
breaks can occur.

*Sensitivity dominance.* The tornado chart (@fig:tornado) shows the
renovation rate dominates all other parameters by 3--6$times$. The
_shape_ of the distribution matters far less than the _bounds_, because
the variance contribution of renovation rate dwarfs other terms regardless
of distributional form.

=== Results

#figure(
  image("figures/fig12_scenario_fan.svg", width: 95%),
  caption: [Monte Carlo fan chart of projected space-heating energy demand,
  2024--2050. Shaded bands show the 10th--90th and 25th--75th percentile
  envelopes from 1,000 iterations. Coloured lines show the three named
  scenarios.],
) <fig:scenario-fan>

@fig:scenario-fan shows the fan chart with the three named scenarios
overlaid on the Monte Carlo uncertainty envelope. The BAU scenario (1%
renovation rate) remains near the upper bound of the distribution, while
the Ambitious scenario (3%) tracks the lower bound. The median trajectory
projects approximately 30% reduction by 2050 relative to the 2023 baseline.
...
The renovation rate is the dominant lever: varying it from 1% to 3%/yr
swings 2050 energy demand by approximately $plus.minus$ 12,600 TJ, while
population uncertainty contributes $plus.minus$ 4,500 TJ and HDD decline
$plus.minus$ 2,000 TJ. The policy-controllable parameter matters far more
than the exogenous drivers.
...
#insight[
  ...
  growth at historical rates will increasingly erode efficiency gains,
  particularly as the "easy wins" in envelope renovation are exhausted and
...
#figure(
  table(
    ...
    [$approx$ 88,000],
    [$-36%$],
    [Renovation rate doubles to EP2050+ target; MFH HP barriers partially removed],
...
this corridor: BAU overshoots by ~1.6--2$times$, Accelerated by 35--75%,
and even Ambitious falls short by 20--55%. Even at 3%/yr renovation rate,
...
(bridging roughly half the gap), accelerated MFH heat-pump deployment
...
rates for MFH systems (e.g., 50% top-up for >4 dwelling units); (iii)
...
*Reference:* cantonal spread from 7.4% (Basel-Stadt) to 41.8% (Fribourg)
...
*Scope:* limited to the 5--10% of stock that cannot be electrified by 2050
...
*Scope.* This analysis covers space heating only; hot water (~15%), cooling,
